...
def _infer_sql_date_range(
    ...
    valid_from = pd.Timestamp(valid_from_arg) if valid_from_arg else dates.min().normalize()
    valid_to = (
        pd.Timestamp(valid_to_arg)
        if valid_to_arg
        else dates.max().normalize() + pd.Timedelta(days=1)
    )
    if pd.isna(valid_from) or pd.isna(valid_to) or valid_from >= valid_to:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.weather:
        ...
        source_info = {"weather_source": "file", "weather_path": str(Path(args.weather))}
    else:
        ...
        valid_from, valid_to = _infer_sql_date_range(
            energy, args.valid_from, args.valid_to_exclusive
        )
        ...
        print(
            "Pobieram pogodę z "
            f"{settings.server} / {settings.database}: {valid_from} <= valid time < {valid_to}"
        )
        ...
        sql_quality = pd.DataFrame(
            [
                ...
                ["sql_valid_from", 0, str(valid_from)],
    ...
    model_quality = pd.DataFrame(
        [
            ...
            ["wiersze_prognozy_przyszlej", int(predictions["status_predykcji"].eq("PROGNOZA_PRZYSZLA").sum()), ""],
